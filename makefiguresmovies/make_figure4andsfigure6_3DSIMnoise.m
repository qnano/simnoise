...
figuredir = ['C:\Users\' netid '\Documents\Structured Illumination Microscopy\SIM noise\figures and movies\Figure4 - noise control in 3D\'];
...
allSNVrecons = zeros(Nx,Ny,Nz,numrecons+1,numdatasets);
for jdataset = 1:numdatasets
  ...
  for jrecon = 1:numrecons
    ...
    allSNVrecons(:,:,:,jrecon,jdataset) = SNVrecon;
  end
...
method = 'signal';
allnoisefractions = zeros(Nx,Ny,Nz,numrecons+1,numdatasets);      
for jdataset = 1:numdatasets
  for jrecon = 1:numrecons
    image_in = allSIMrecons(:,:,:,jrecon,jdataset);
    SNV_in = allSNVrecons(:,:,:,jrecon,jdataset)/sum(image_in(:)); % normalize SNV by sum 3D image signal
    ...
    for jz = 1:Nz
      ...
      [~,allnoisefractions(:,:,jz,jrecon,jdataset),~,~] = get_noisefraction(image_slice,SNV_slice,SIMparams.SIMpixelsize(1),gausswidpar,method);
    end
  end
end
...
for jdataset = 1:numdatasets
  ...
  for jrecon = 1:numrecons+1
    ...
    maxval = max(tempim_xy(:));
    minval = min(tempim_xy(:));
    tempim_xy = (tempim_xy-minval)/(maxval-minval);
    maxval = max(tempim_xz(:));
    minval = min(tempim_xz(:));
    tempim_xz = (tempim_xz-minval)/(maxval-minval);
    
    noisefraction = squeeze(allnoisefractions(:,:,:,jrecon,jdataset));
    tempnoise_xy = squeeze(noisefraction(cropX,cropY,showfocus));
    tempnoise_xz = squeeze(noisefraction(cropX,showcol,:))';
    
    tempim_xy_noisemap = zeros(length(cropX),length(cropY),3);
    tempim_xy_noisemap(:,:,1) = tempnoise_xy;
    tempim_xy_noisemap(:,:,2) = tempim_xy;
    tempim_xy_noisemap(:,:,3) = tempnoise_xy;
    tempim_xz_noisemap = zeros(Nz,length(cropX),3);
    tempim_xz_noisemap(:,:,1) = tempnoise_xz;
    tempim_xz_noisemap(:,:,2) = tempim_xz;
    tempim_xz_noisemap(:,:,3) = tempnoise_xz;
    
    % plot xy-slice
    figure
    set(gcf,'units','pixels');
    set(gcf,'Position',normfac*[0.0+jdataset*10.0 24.0-jrecon*4.0  1.5*figsizeunit 1.5*figsizeunit]);
    imagesc(tempim_xy);
    colormap(mappy)
%     image(tempim_xy_noisemap);
    ...
    set(gcf,'Position',normfac*[4.0+jdataset*10.0 27.0-jrecon*4.0  1.5*figsizeunit 1.5*aspectratiofac*figsizeunit]);
    imagesc(tempim_xz);
    colormap(mappy)
%     image(tempim_xz_noisemap);
    axis off
    set(gca,'position',[0 0 1 1],'units','normalized')
