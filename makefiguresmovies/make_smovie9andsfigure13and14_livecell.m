...
moviedir = ['C:\Users\' netid '\Documents\Structured Illumination Microscopy\SIM noise\figures and movies\SMovie9 - live cell\'];
figuredir = ['C:\Users\' netid '\Documents\Structured Illumination Microscopy\SIM noise\figures and movies\SFigure13 - live cell\'];
...
allSIMrecons = zeros(Nx,Ny,numSIMfocus,numchannels,numframes,numrecons);
allSNVrecons = zeros(Nx,Ny,numSIMfocus,numchannels,numframes,numrecons);
...
StateOfArt_OMX_combi = zeros(Nx,Ny,numSIMfocus,numframes);
...
method = 'signal';
allnoisefractions = zeros(Nx,Ny,numSIMfocus,numframes,numrecons);
jchannel = 1;
for jrecon = 1:numrecons
  for jframe = 1:numframes
    image_in = allSIMrecons(:,:,:,jchannel,jframe,jrecon);
    SNV_in = allSNVrecons(:,:,:,jchannel,jframe,jrecon)/sum(image_in(:)); % normalize SNV by sum 3D image signal
    SNV_in = mean(SNV_in,3); % average SNV over axial direction
    for jz = 1:numSIMfocus
      ...
      [~,allnoisefractions(:,:,jz,jframe,jrecon),~,~] = get_noisefraction(image_slice,SNV_slice,SIMparams.SIMpixelsize(1),gausswidpar,method);
    end
  end
end

widefield_noisemap_combi = zeros(Nx,Ny,Nz,numframes);
StateOfArt_noisemap_combi = squeeze(allnoisefractions(:,:,cropZ,:,1));
TrueWiener_noisemap_combi = squeeze(allnoisefractions(:,:,cropZ,:,2));
FlatNoise_noisemap_combi = squeeze(allnoisefractions(:,:,cropZ,:,3));
NotchFiltering_noisemap_combi = squeeze(allnoisefractions(:,:,cropZ,:,4));

max(StateOfArt_noisemap_combi(:))
max(TrueWiener_noisemap_combi(:))
max(FlatNoise_noisemap_combi(:))
max(NotchFiltering_noisemap_combi(:))
...
numreconshow = 5; % number of reconstruction rows in movie object

makenoisemap = 1; % flag for plotting noise fraction map in combination with reconstruction
...
for jframe = 1:numframes
  ...
  if makenoisemap
    tempim_combi = zeros(Nz*length(cropX),numreconshow*length(cropY),3);
  else
...
%     tempframe_no = (tempframe_no-minval_no(jframe))/(maxval_no(jframe)-minval_no(jframe));
  % noise map slices
    if makenoisemap
      tempframe_noisemap_wf = widefield_noisemap_combi(cropX,cropY,jz,jframe);
      tempframe_noisemap_sa = StateOfArt_noisemap_combi(cropX,cropY,jz,jframe);
      tempframe_noisemap_tw = TrueWiener_noisemap_combi(cropX,cropY,jz,jframe);
      tempframe_noisemap_fn = FlatNoise_noisemap_combi(cropX,cropY,jz,jframe);
      tempframe_noisemap_no = NotchFiltering_noisemap_combi(cropX,cropY,jz,jframe);
    end
  % make image tile
    if makenoisemap
      tempim_combi((1+(jz-1)*length(cropY)):jz*length(cropY),:,1) = horzcat(tempframe_noisemap_wf,tempframe_noisemap_sa,tempframe_noisemap_tw,tempframe_noisemap_fn,tempframe_noisemap_no);
      tempim_combi((1+(jz-1)*length(cropY)):jz*length(cropY),:,2) = horzcat(tempframe_wf,tempframe_sa,tempframe_tw,tempframe_fn,tempframe_no);
      tempim_combi((1+(jz-1)*length(cropY)):jz*length(cropY),:,3) = horzcat(tempframe_noisemap_wf,tempframe_noisemap_sa,tempframe_noisemap_tw,tempframe_noisemap_fn,tempframe_noisemap_no);
    else
%       tempim_combi(:,(1+(jz-1)*length(cropY)):jz*length(cropY)) = vertcat(tempframe_wf,tempframe_tw,tempframe_fn,tempframe_no);
      tempim_combi((1+(jz-1)*length(cropY)):jz*length(cropY),:) = horzcat(tempframe_wf,tempframe_sa,tempframe_tw,tempframe_fn,tempframe_no);
    end
  end
...
%     annotation('textbox',[0.0 0.15 0.1 0.1],'String','notch-filtered SIM','FontSize',16,'Edgecolor','none','Color','white');
    ...
    annotation('textbox',[0.2 0.93 0.1 0.07],'String','state-of-art SIM','FontSize',10,'Edgecolor','none','Color','white','HorizontalAlignment','left');
    annotation('textbox',[0.4 0.93 0.1 0.07],'String','true-Wiener SIM','FontSize',10,'Edgecolor','none','Color','white','HorizontalAlignment','left');
    annotation('textbox',[0.6 0.93 0.1 0.07],'String','flat-noise SIM','FontSize',10,'Edgecolor','none','Color','white','HorizontalAlignment','left');
    annotation('textbox',[0.8 0.93 0.1 0.07],'String','notch-filtered SIM','FontSize',10,'Edgecolor','none','Color','white','HorizontalAlignment','left');
...
%     annotation('textbox',[0.014 0.04 2*width 0.04],'String',scalebarstring,'FontSize',16,'Edgecolor','none','Color','white');
    annotation('rectangle',[0.505 0.01 width 0.01],'FaceColor','white','Color','white');
...
%     glancm = annotation('textbox',[0.45 0.93 0.25 0.065],'String',stringyt,'FontSize',16,'Edgecolor','none','Color','white');
  ...
  if jframe==10
    if makenoisemap
      savefilename = strcat(figuredir,'snapshot_combi_noisemap.svg');
    else
...
for jframe = 1:numframes
  ...
  % loop over focal slices to compile and scale color images
  tempim_combi = zeros(3*length(cropX),2*length(cropY));
  ...
  % make image tile
  tempim_combi = [tempframe_wf,tempframe_sa,tempframe_tw;tempframe_fn,tempframe_no,zeros(size(tempframe_no))];
      
  % make movie              
  figure(35);
  set(gcf,'Position',round([0.10*scrsz(3) 0.05*scrsz(4) 1.2*scrsz(4) 0.8*scrsz(4)]));
  imagesc(tempim_combi)
  colormap(mappy);
  axis off
  axis tight
  if jframe==1
    annotation('textbox',[0.0 0.90 0.1 0.1],'String','widefield','FontSize',12,'Edgecolor','none','Color','white');
    annotation('textbox',[0.33 0.90 0.1 0.1],'String','state-of-art SIM','FontSize',12,'Edgecolor','none','Color','white');
    annotation('textbox',[0.68 0.90 0.1 0.1],'String','true-Wiener SIM','FontSize',12,'Edgecolor','none','Color','white');
    annotation('textbox',[0.0 0.40 0.1 0.1],'String','flat-noise SIM','FontSize',12,'Edgecolor','none','Color','white');
    annotation('textbox',[0.33 0.40 0.1 0.1],'String','notch-filtered SIM','FontSize',12,'Edgecolor','none','Color','white');
    annotation('rectangle',[0.02 0.02 width 0.01],'FaceColor','white','Color','white');
    annotation('textbox',[0.015 0.04 2*width 0.04],'String',scalebarstring,'FontSize',16,'Edgecolor','none','Color','white');
  end
  set(gca,'position',[0 0 1 1],'units','normalized')
  stringyt = strcat('t = ',num2str(allt(jframe),'%3.0f'),' ms');
  if ~ishandle(glancm)
    glancm = annotation('textbox',[0.68 0.43 0.25 0.065],'String',stringyt,'FontSize',16,'Edgecolor','none','Color','white');
  else
    glancm.String = stringyt;
  end
  set(gca,'position',[0 0 1 1],'units','normalized')
