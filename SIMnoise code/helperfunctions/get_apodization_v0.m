...
% % compute the axial spatial frequency of the extended SIM OTF cutoff as a
% % function of lateral spatial frequency "axcutoff", subsequently the
% % axial distance "axdist" of each 3D spatial frequency vector within the
% % SIM OTF support to the extended SIM cutoff
% allaxcutoff = zeros(Nx,Ny,Nz,numorders,numangles);
% for jangle = 1:numangles
%   qvector = [cos(patternangles(jangle)) sin(patternangles(jangle))]/patternpitch(jangle);
%   % compute axial shift of two branches of 1st order, at +/-kz
%   q0ex = refmed/lambdaex;
...
%     if mod(mm,2)==1
%       axialcutoff = axialcutoff+axialshift;
%     end
%     allaxcutoff(:,:,:,jorder,jangle) = double(qpar<=2*NAl).*axialcutoff;
%   end
% end
% unionaxcutoff = max(max(allaxcutoff,[],5),[],4); % union of axial cutoff spatial frequencies
% unionaxcutoff(unionaxcutoff<epsy) = 0;
% axdist = unionaxcutoff-abs(qz); % distance spatial frequency vector to union axial cutoff
% axdist(axdist<epsy) = 0;

%%
% % compute a function that satisfies Fcutoff = abs(q) at the 3D SIM cutoff
% % surface using the previously computed function axdist
% % Fcutoff = qrad+axdist;
% Fcutoff = sqrt(qx.^2+qy.^2+(abs(qz)+axdist).^2);
% 
% % compute the triangle filter and a simplified form of the Lukosz bound,
% % using the function Fcutoff
% %
% % ... in 2D this is a poor approximation to a triangle filter, needs a real
% % solution, better to re-instate the line length to the cutoff as measure
% % for the cutoff as a function of orientation ...
% LukoszBound = cos((pi/2)*qrad./Fcutoff);
% TrianglexFilter = 1-qrad./Fcutoff;
...
Nazim = 32*numangles;
Npola = min(4*Nz+1,97); % guarantees an odd number which is needed for having one sample in qz=0 plane, which is needed for consistency with 2D
...
% compute the polar and azimuthal indices for each 3D-spatial frequency
